...
const MarkAsDeliveredFailedModal = ({open, error, tryAgain, closeModal} : {open: boolean; tryAgain: () => void; closeModal: () => void; error: {message: string; error: string}| null}) => {
  return (
    <motion.div className={`${open ? 'flex' : 'hidden'} w-[100vw] h-dvh fixed top-0 left-0 items-center justify-center bg-transparent z-200`}>
        ...
        className="w-full max-w-xs bg-[#F8FAFF] rounded-xl p-6 z-20 relative shadow-md">
            ...
            <h3 className='text-black font-raleway font-semibold text-xl mt-5 text-center max-w-[250px]'>Failed to Mark Item as Delivered</h3>
...
export default MarkAsDeliveredFailedModal;